...
| TurboQuote (`quote.ts`) | ~52 | 0 | entire module |
| TurboPartner (`partner.ts`) | ~30 | 0 | entire module (needs 2nd credential) |
| TurboWebhooks (`webhooks.ts`) | 10 + `verifyWebhookSignature` | 0 | entire module |
...
| SDK method | HTTP | n8n resource/operation | Status |
|---|---|---|---|
| `listDeliverables(opts)` | GET `/v1/deliverable` | deliverable / `list` | ❌ → **adding** |
| `generateDeliverable(req)` | POST `/v1/deliverable` | deliverable / `generate` | ❌ → **adding** |
| `getDeliverableDetails(id, opts)` | GET `/v1/deliverable/{id}` | deliverable / `get` | ❌ → **adding** |
| `updateDeliverableInfo(id, req)` | PATCH `/v1/deliverable/{id}` | deliverable / `update` | ❌ → **adding** |
| `deleteDeliverable(id)` | DELETE `/v1/deliverable/{id}` | deliverable / `delete` | ❌ → **adding** |
| `downloadSourceFile(id)` | GET `/v1/deliverable/file/{id}` (binary) | deliverable / `downloadSource` | ❌ → **adding** |
| `downloadPDF(id)` | GET `/v1/deliverable/file/pdf/{id}` (binary) | deliverable / `downloadPdf` | ❌ → **adding** |
...
| `verifyWebhookSignature(...)` | local HMAC verify (no API call) | **TurboDocx Trigger node** (auto-registers + verifies) | ❌ → **adding (beyond strict parity)** |
